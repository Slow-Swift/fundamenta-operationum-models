...
export class ModelGui {
  ...
  addSlider(label, object={}, field, min=0, max=90, { step=0.1, formatter=degreeFormatter } = {}) {
...
labelSpan.innerText = `${label}: `;
    ...
    input.oninput = (e) => {
      katex.render(formatter ? formatter(input.value) : input.value, valueSpan);
    ...
    slider.setRange = function (min, max) {
      ...
      katex.render(formatter ? formatter(input.min) : input.min, startLabel);
      katex.render(formatter ? formatter(input.max) : input.max, endLabel);
  ...
  addToggle(label, object={}, field) {
    const toggle = document.createElement('label');
    toggle.classList.add('toggle');

    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = object[field];

    input.oninput = (e) => {
      if (input.checked == object[field]) return;

      object[field] = input.checked;
      this.onInputChanged?.();
    }
    ...
    this.sliderArea.appendChild(toggle);
    this.sliders.push(toggle);
